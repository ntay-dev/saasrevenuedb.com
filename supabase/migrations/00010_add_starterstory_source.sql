-- ============================================================
-- Migration 00010: Add StarterStory (YouTube) source to transform_bronze_to_silver
-- ============================================================
-- StarterStory YouTube: ~143 founder interview videos with revenue data
-- extracted from transcripts. Trust level 50 (self-reported in video).
  ...
  LOOP
    BEGIN
      ...
      ELSIF v_record.source_name = 'starterstory' THEN
        -- Use product name as slug if available, otherwise video ID
        ...
        ELSE
          v_slug := 'ss-' || v_record.external_id;
        END IF;
      ELSE
      ...
      ELSIF v_record.source_name = 'starterstory' THEN
        v_name := COALESCE(v_raw->>'productName', v_record.external_id);
      ELSE
